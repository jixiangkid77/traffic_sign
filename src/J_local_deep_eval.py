...
# All 12 official challenge types (was [4,8,9,11,12]). With --resume the 5
# already-cached challenges are skipped by filename; only the 7 new ones run.
...
def verify_determinism(args):
    """Re-run the deep model + classifier for one already-cached cell and compare
    pred/prob byte-for-byte against deep_<model>_cure.csv. A mismatch means the
    local environment (weights / torch / einops / opencv) drifted since the cached
    run -- STOP and regenerate all 12 challenges fresh. Run this per model BEFORE
    the 12-class --resume sweep."""
    out_csv = OUT_DIR / f"deep_{args.model}_cure.csv"
    if not out_csv.exists():
        sys.exit(f"[FATAL] cache not found: {out_csv}")
    try:
        vch, vsev = (int(x) for x in str(args.verify_cell).split(","))
    except Exception:
        sys.exit("[FATAL] --verify-cell must be 'ch,sev', e.g. 12,5")
    ...
    with open(out_csv, encoding="utf-8", newline="") as f:
        for r in csv.DictReader(f):
            if int(r["ch_id"]) == vch and int(r["sev"]) == vsev:
                cached[r["filename"]] = r
    if not cached:
        sys.exit(f"[FATAL] no cached rows for cell ch={vch} sev={vsev} in "
                 f"{out_csv.name}; pick a cached cell with --verify-cell.")
    print(f"[determinism] {args.model} cell ch={vch}"
          f"({CHALLENGE_TYPES.get(vch,'?')}) sev={vsev}: "
          f"{len(cached)} cached rows to re-verify")

    cure_root = Path(args.cure_root)
    samples, _ = collect_samples(cure_root)
    samples = [s for s in samples
               if s["ch"] == vch and s["sev"] == vsev and s["filename"] in cached]
    if not samples:
        sys.exit("[FATAL] could not find the cell's images under "
                 f"{cure_root}; point --cure-root at the cached run's folder.")

    device = "cpu"
    if args.threads > 0:
        torch.set_num_threads(args.threads)
    if args.model == "adair":
        net, _ = load_adair(device, args.adair_weight)
    else:
        net, _ = load_cidnet(device, args.cidnet_weight)
    model = load_gtsrb_compactcnn(device)
    tfm = build_transform()

    # reload cell rows keeping duplicate filenames (Real_Train/Real_Test collisions)
    from collections import defaultdict as _dd
    PROB_TOL = 1e-3   # softmax prob may differ by ~1e-4 due to CPU float reduction
    cached_by_fn = _dd(list)
    with open(out_csv, encoding="utf-8", newline="") as f:
        for r in csv.DictReader(f):
            if int(r["ch_id"]) == vch and int(r["sev"]) == vsev:
                cached_by_fn[r["filename"]].append(
                    (int(r["pred"]), float(r["prob"])))

    keep_m, keep_i = [], []
    for s2 in samples:
        im = cv2.imread(str(s2["path"]))
        if im is not None:
            keep_m.append(s2); keep_i.append(im)

    fresh_by_fn = _dd(list)
    for cm, ci in same_size_batches(keep_m, keep_i, args.batch):
        enh = enhance_batch(net, ci, device)
        preds, probs = classify_batch(model, enh, tfm, device)
        for kk, m in enumerate(cm):
            fresh_by_fn[m["filename"]].append(
                (int(preds[kk]), float(probs[kk])))

    n_fn = n_fn_ok = 0; n_rows = n_rows_ok = 0
    n_pred_bad = 0; max_dprob = 0.0; bad = []
    for fn in set(cached_by_fn) | set(fresh_by_fn):
        cl = sorted(cached_by_fn.get(fn, []))
        fl = sorted(fresh_by_fn.get(fn, []))
        n_rows += max(len(cl), len(fl))
        n_fn += 1
        ok = (len(cl) == len(fl))
        if ok:
            for (cp, cpr), (fp_, fpr) in zip(cl, fl):
                d = abs(cpr - fpr)
                if d > max_dprob:
                    max_dprob = d
                if cp != fp_:
                    ok = False; n_pred_bad += 1
                elif d > PROB_TOL:
                    ok = False
        if ok:
            n_fn_ok += 1; n_rows_ok += len(cl)
        elif len(bad) < 8:
            bad.append((fn, cl, fl))

    print(f"\n=== DETERMINISM CHECK (J, {args.model}; per-(filename,occurrence)) ===")
    print(f"  distinct filenames in cell : {n_fn}")
    print(f"  filenames matching         : {n_fn_ok}")
    print(f"  filenames differing        : {n_fn - n_fn_ok}")
    print(f"  image-rows compared        : {n_rows}   matched: {n_rows_ok}")
    print(f"  predictions differing      : {n_pred_bad}   (argmax stability)")
    print(f"  max |prob deviation|       : {max_dprob:.2e}   (tol {PROB_TOL:.0e})")
    if bad:
        print("  first differing (filename | cached (pred,prob) | fresh):")
        for fn, cl, fl in bad:
            print(f"    {fn}  {cl}  ->  {fl}")
    if n_fn > 0 and n_fn_ok == n_fn:
        print("\n  [OK] environment STABLE: every prediction matches and prob")
        print("       deviations are below tolerance (pure CPU float noise). Safe")
        print("       to run the 12-class sweep with --resume (appends the 7 new")
        print("       challenges only).")
    elif n_pred_bad == 0:
        print("\n  [OK-with-note] all PREDICTIONS match; only softmax probs drift")
        print(f"       slightly (max {max_dprob:.2e}). This is benign CPU float")
        print("       non-determinism, not a weight/code change; accuracy and the")
        print("       cascade (0.05-grid tau) are unaffected. Safe to proceed.")
    else:
        print(f"\n  [STOP] {n_pred_bad} PREDICTIONS changed -- real drift (weights/code/")
        print("         libs). Do NOT append; send me this output.")
...
def main():
    ...
    ap.add_argument("--verify-determinism", action="store_true",
                    help="re-run one already-cached cell through the deep "
                         "model and diff pred/prob against "
                         "deep_<model>_cure.csv to detect environment "
                         "drift before appending 7 new challenges")
    ap.add_argument("--verify-cell", default="12,5",
                    help="challenge,severity cell for --verify-determinism "
                         "(must already be in that model's cache; default "
                         "Haze sev5)")
